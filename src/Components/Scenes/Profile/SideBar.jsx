import { Avatar, Box, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Typography, useMediaQuery, useTheme } from '@mui/material'
...
function SideBar({ handleItem, selected }) {
    ...
    return (
        <Box p={isMatchedTablette ? '4px 10px' : '10px 30px'} height='calc(100vh - 100px)' borderRight='solid 1.5px grey'>
            <Box display='flex' flexDirection={isMatchedTablette ? 'column' : 'row'} alignItems='center' mb={5} >
                <img src={user} alt="profile_name"  width={isMatchedTablette ? '55px' : '70px'} height={isMatchedTablette ? '55px' : '75px'}style={{ objectFit:'cover',marginRight: isMatchedTablette ? '0px' : '15px', marginBottom: isMatchedTablette ? '10px' : '0px', borderRadius: '18px' }} />
